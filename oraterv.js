...
const S=()=>Y.state();
const esc=s=>Y.esc(s);
function migrate(s){const o=s.oraterv&&typeof s.oraterv==='object'?s.oraterv:{};o.start=o.start||'';o.weeks=Math.max(1,Math.min(52,Number(o.weeks)||15));o.rows=Array.isArray(o.rows)?o.rows:[];o.rows.forEach(r=>{r.plan=Array.isArray(r.plan)?r.plan.map(x=>Number(x)||0):[];while(r.plan.length<o.weeks)r.plan.push(0);r.plan.length=o.weeks});s.oraterv=o}
...
function view(){
  ...
  return head+`<div class="card">${info}<div class="ot-wrap"><table class="ot"><thead>${thead}</thead><tbody>${rows}${sum}</tbody></table></div>${add}<p class="vow-note" style="margin:10px 0 0">Cella: <b>terv</b> óra, alatta a <small>megvolt</small>. Zöld = a heti keret megvolt, sárga = félig, piros = elmaradt; a mostani hét kiemelve. A megvolt a szokásra naplózott percekből (pomodoro, napiterv, kézi beírás) jön.</p></div>`;
}
function bind(){

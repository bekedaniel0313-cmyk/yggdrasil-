...
function section(s: Json, name: string, date: string) {
  ...
  if (n === "rewards" || n === "ajandekok") return (s.rewards || []).map((r: Json) => `- ${r.unlockedAt ? "✨ " : ""}${r.name} — ${r.collected}/${r.fragments} fregment${r.price ? ` · ${r.price} Ft` : ""}`).join("\n");
